    ...
    with zipfile.ZipFile(zip_path, 'w') as zipf:
        ...
        for root, dirs, files in os.walk(source_folder):
            for file in files:
                ...
                rel_path_without_accents = unidecode(rel_path)  # Remove accents
                zipf.write(file_path, rel_path_without_accents)

    # Move the zip file to the destination folder, overwriting if it already exists
    try: 
        os.replace(zip_path, os.path.join(destination_folder, zip_filename))
    except FileNotFoundError: 
        pass  # File does not exist yet, no need to overwrite
